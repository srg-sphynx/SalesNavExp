    ...
    try {
        while (allLeads.length < maxLeads) {
            ...
            if (leads.length === 0) {
                ...
                let message = 'No leads found on this page.';
                if (!currentUrl.includes('linkedin.com/sales')) {
                    message = 'No leads found — make sure you are viewing a Sales Navigator list, not a regular LinkedIn page.';
                } else if (!currentUrl.includes('/lists/') && !currentUrl.includes('/search/')) {
                    message = 'No leads found — navigate to a Sales Navigator people list or search results page, then try again.';
                } else {
                    message = 'No leads found. The page may still be loading, or LinkedIn may have updated their layout. Try refreshing and scraping again.';
                }
                onIssue({ type: 'warning', message });
                break;
            }
            ...
            for (let i = 0; i < leads.length && allLeads.length < maxLeads; i++) {
                ...
                await humanDelay(delayBetweenLeads, 0.3);
            }
